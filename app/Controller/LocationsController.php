 ...
 */
	public function admin_delete($id = null) {
		...
		$apptChk=$this->Appointment->find('first', array('conditions' => array('locationid' => $id)));
		if(count($apptChk)>0){
			$this->Session->setFlash(__('to Delete this Location  First delete its related data or <a href="http://'.$_SERVER['HTTP_HOST'].Router::url('/admin/Locations/deleterelate/').$id.'" style="color:#ff0000">click here</a> to delete its related data and that Location'));
 ...
 */
	public function admin_deleterelate($id = null) {
		...
		$apptChk=$this->Appointment->find('first', array('conditions' => array('locationid' => $id)));
		if(count($apptChk)>0){
			$diagTestChk=$this->Appointment->deleteAll(array('locationid' => $id));
		}
