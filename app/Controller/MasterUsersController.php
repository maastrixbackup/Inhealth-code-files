 ...
 */
	public function admin_delete($id = null) {
		...
		$this->loadModel('Appointment');
			...
			$fulltimeAppoint=$this->RegularAppointment->find('first', array('conditions' => array('patientid' => $id)));
			$this->loadModel('Feedback');
			$feedbacks=$this->Feedback->find('first', array('conditions' => array('user_id' => $id)));

			
			if(count($apptChk)>0 || count($dChk)>0 || count($testChk)>0 || count($fulltimeAppoint)>0 || count($feedbacks)>0){
	...
	public function admin_deleterelate($id = null) {
		...
		$this->loadModel('Appointment');
			...
			if(count($fulltimeAppoint)>0){
				$deleteRegularApnt=$this->RegularAppointment->deleteAll(array('patientid' => $id));
			}
			if(count($feedbacks)>0){
				$deleteRegularApnt=$this->Feedback->deleteAll(array('user_id' => $id));
			}
