 ...
 */
	public function AddAppointment(){
		...
		$availabilityList = array('' => 'Select Time');
		$availabilityList+= $this->DoctorAvailability->find('list', array('conditions' => array('NOT'=>array('id'=>$avaialbleID),'DoctorAvailability.status' => 1,'date(DoctorAvailability.app_date)' => $appointDate, 'DoctorAvailability.doctor_id' => $docID), 'order' => array('created' => 'asc'), 'fields' => array('id', 'name')));
		$this->set('availabilityList', $availabilityList);
 ...
 */
	public function uploadtestlist(){
		...
		$this->loadModel('UploadtestResult');
		if($loginType=="P"){
			$testReportDet=$this->UploadtestResult->find('all', array('conditions' => array('userid' => $loginID ,'status'=>1),'order'=>array('id'=>'desc')));
		}else if($loginType=="D"){
			$testReportDet=$this->UploadtestResult->find('all', array('conditions' => array('doctorid' => $loginID , 'status'=>1),'order'=>array('id'=>'desc')));
		}
