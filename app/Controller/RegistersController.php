 ...
 */
	public function patient() {
		...
		if ($this->request->is('post')) {
			...
			//else if($_SESSION['6_letters_code'] != trim($this->request->data['Register']['captcha_text']) ){
				//$this->Session->setFlash(__('Captcha donot match.Please refresh captcha and Try Again!'));				
				//}
				else{
 ...
 */
	public function doctor() {
		...
		if ($this->request->is('post')) {
			...
			$uname=$this->request->data['Register']['user_name'];
			$email=$this->request->data['Register']['email_id'];
			$checkEmail=$this->Register->find('first', array('conditions' => array('email_id' => $email,'login_tytpe'=>'D')));
			$checkUname=$this->Register->find('first', array('conditions' => array('user_name' => $uname)));
 ...
 */
	public function hospital() {
		...
		if ($this->request->is('post')) {
			...
			$uname=$this->request->data['Register']['user_name'];
			$email=$this->request->data['Register']['email_id'];
			$checkEmail=$this->Register->find('first', array('conditions' => array('email_id' => $email,'login_tytpe'=>'H')));
			$checkUname=$this->Register->find('first', array('conditions' => array('user_name' => $uname)));
	...
	public function pharmasist() {
		...
		if ($this->request->is('post')) {
			...
			//Reference ID generation============
			$lastUser = $this->Register->find('first', array('conditions' => array('login_tytpe' => 'PH'), 'order' => array('id' => 'desc')));
			if(count($lastUser)>0){
				$userLastID = $lastUser['Register']['id'];
				$lastid=$lastUser['Register']['ref_id'];
			...
			$this->request->data['Register']['login_tytpe']='PH';
			...
			$email=$this->request->data['Register']['email_id'];
			if($email!=''){
			$checkemail=$this->Register->find('first', array('conditions' => array('email_id' => $email ,'login_tytpe'=>'PH')));
		}else{$checkemail=array();}
			$checkUname=$this->Register->find('first', array('conditions' => array('user_name' => $uname)));
			if($pwdData!=$confPwd){
				$this->Session->setFlash(__('Mismatch in password. Please, try again.'));
			...
			//else if($_SESSION['6_letters_code'] != trim($this->request->data['Register']['captcha_text']) ){
				...
				$this->Register->create();
					if ($this->Register->save($this->request->data)) {
						...
						//==================================================================
						$this->Session->setFlash(__('pharmasist has successfully registered&nbsp;&nbsp;<a href="http://'.$_SERVER['HTTP_HOST'].Router::url('/').'">Login</a>'));
						//return $this->redirect(array('action' => 'index'));
					} else {
						$this->Session->setFlash(__('The pharmasist detail could not be saved. Please fill all required field to proceed.'));
					}
				}
	...
	public function lab() {
		...
		if ($this->request->is('post')) {
			...
			//Reference ID generation============
			$lastUser = $this->Register->find('first', array('conditions' => array('login_tytpe' => 'L'), 'order' => array('id' => 'desc')));
			if(count($lastUser)>0){
				$userLastID = $lastUser['Register']['id'];
				$lastid=$lastUser['Register']['ref_id'];
				$lastno = ($lastid!='') ? explode("A",$lastid) : 10000;
			...
			$this->request->data['Register']['login_tytpe']='L';
			...
			$email=$this->request->data['Register']['email_id'];
			if($email!=''){
			$checkemail=$this->Register->find('first', array('conditions' => array('email_id' => $email ,'login_tytpe'=>'L')));
		}else{$checkemail=array();}
			$checkUname=$this->Register->find('first', array('conditions' => array('user_name' => $uname)));
			if($pwdData!=$confPwd){
				$this->Session->setFlash(__('Mismatch in password. Please, try again.'));
			...
			//else if($_SESSION['6_letters_code'] != trim($this->request->data['Register']['captcha_text']) ){
				...
				$this->Register->create();
					if ($this->Register->save($this->request->data)) {
						...
						//==================================================================
						$this->Session->setFlash(__('Lab has successfully registered&nbsp;&nbsp;<a href="http://'.$_SERVER['HTTP_HOST'].Router::url('/').'">Login</a>'));
						//return $this->redirect(array('action' => 'index'));
					} else {
