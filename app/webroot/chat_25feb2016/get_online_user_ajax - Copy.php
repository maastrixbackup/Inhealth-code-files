...
if(isset($_SESSION['user_type']) && $_SESSION['user_type']=='D'){
	$where="doctorid=".$_SESSION['user_id']." AND status=0 AND appointment_date='".date("Y-m-d")."'";
	}
	else
	if(isset($_SESSION['user_type']) && $_SESSION['user_type']=='P'){
	$where="patientid=".$_SESSION['user_id']." AND status=0 AND appointment_date='".date("Y-m-d")."'";
	}
...
$checkofl="update appointments set status=1 where id=".$_SESSION['app_id'];
