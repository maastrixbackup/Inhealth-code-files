...
if($diff > $logoff && $diff1 > $logoff){
	$update_user = mysql_query("UPDATE master_users SET is_online = '0' WHERE user_name = '".$_SESSION['user_name']."' AND user_pass = '".$_SESSION['password']."' AND id = '".$_SESSION['user_id']."' ") or die("Error in master_users Update Logout: ".mysql_error());
...
while ($ol_user123 = mysql_fetch_array($oluser1)) {
	...
	if($diff > $logoff && $diff1 > $logoff){
		$update_user = mysql_query("UPDATE master_users SET is_online = '0' WHERE  id = '".$ol_user123['user_id']."' ") or die("Error in master_users Update Logout: ".mysql_error());
...
}else if(isset($_SESSION['user_type']) && $_SESSION['user_type']=='P'){
	$where="patientid=".$_SESSION['user_id']." AND status=1 AND appointment_date='".date("Y-m-d")."'";
}

#==Sql command===#
//echo "appoint_id:".$_SESSION['appoint_id'];exit;
$chk_app="select * from appointments where id=".$_SESSION['appoint_id'];
$chk_process=mysql_query($chk_app);
if(mysql_num_rows($chk_process)>0){
	$chk_app_success=mysql_fetch_array($chk_process);
	
	$gettime="select * from `doctoravailable_slots` where id=".$chk_app_success['appoint_book_slut'];
...
}else if(isset($_SESSION['loginType']) && $_SESSION['loginType']=='P'){

	$chatuser=$chk_app_success['patientid'];
	$rchatuser=$chk_app_success['doctorid'];
}
#===checking condition end is doctor or patient====#
//echo "cur time:".$cur_time."session start time: ".$_SESSION['start_time']."=== End time".$_SESSION['end_time'];exit;
...
?>
	...
	$checkofl="update appointments set join_status=3 where id=".$_SESSION['appoint_id'];
	...
	echo "<script>alert('Now you are Timed Out for appointment id ".$_SESSION['appoint_id']."');window.top.location.href='".SITEURL."/dashboards';</script>";
