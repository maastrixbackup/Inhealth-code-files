<?php 
/*function checkdisableuser($id){
$usersql=mysql_query("select * from regular_appointments where id= '".$id."' and is_conv='2' and status='1' and is_connected='0");	
$timeoutuser=mysql_fetch_array($usersql);
if($_SESSION['user_id']==$timeoutuser['patientid'] && $timeoutuser['is_conv']==2){
...
checkdisableuser($_SESSION['app_id']);*/
...
<!--<div id="tabs" style="max-height:343px;background-color:#e3efff">-->
<div id="tabs" style="max-height:auto;">

            <ul id="chat_tabs">
            <?php 
              foreach ($chat_user as $key => $id) {
                if ($id != '') {
                  $username_q = mysql_query("SELECT * FROM `master_users` WHERE `status` = 1 AND `is_online` = '1' AND `is_conv` = '0' AND id = $id");
                  $username_q = mysql_fetch_object($username_q);
				  if($username_q->user_name!="") {
                  ?>
				  
                            <li onclick="ChangeStatus123('<?php echo $id ;?>','<?php echo $_SESSION['app_id'];?>');" class="chat_tab<?php echo $id ;?> <?php if ($_SESSION['corent_user']==$id): ?> ui-corner-top ui-tabs-active ui-state-active<?php endif ?>" id="<?php echo $id ;?>">
                            <a href="#tabs-<?php echo $id ;?>"><?php echo $username_q->user_name;?></a>&nbsp; <a class="close_tab" id="<?php echo $id ;?>" href="?click"><span <?php if($_SESSION['user_type']=='D'){?>onclick="ChangeStatus123('<?php echo $id ;?>','<?php echo $_SESSION['app_id'];?>');"<?php }?> class="glyphicon glyphicon-remove-circle"></span></a></li>
                  <?php
				  }
                }
              }
  ...
  <div id="message_con">
   ...
   <?php 
              foreach ($chat_user as $key => $id) {
                if ($id != '') {
                  $username_m = mysql_query("SELECT * FROM `master_users` WHERE `status` = 1 AND `is_online` = '1' AND `is_conv` = '0' AND id = $id") or die(mysql_error());
...
<!--<div class="chatboxtitle">chitta</div>-->
<div class="chatboxoptions">
<!--<a href="javascript:void(0)" onclick="javascript:toggleChatBoxGrowth('chitta')">-</a> 
<a href="javascript:void(0)" onclick="javascript:closeChatBox('chitta')">X</a>--></div>
<!--<br clear="all"><a href="javascript:void(0)" onclick="javascript:webcam('chitta');">
<button type="button" id="Sendmessage"  name="Sendmessage" class="btn btn-xs btn-success " onclick="javascript:webcam('<?php //echo $username_q->user_name;?>');">
webcam &nbsp;
<img src="images/webcam-icon.png"></button>&nbsp;<!--</a>-->
<!-- <a href="javascript:void(0)" onclick="javascript:block('chitta');">Block</a> --></div>
  
  </td>
  </tr>
  
                           <tr>
    <td height="38">      <button type="button" id="Sendmessage"  name="Sendmessage" class="btn btn-xs btn-primary ">Send &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="glyphicon glyphicon-send"></span></button>&nbsp;</td>
  </tr>
  <tr>
    <td height="38">
 <button id="trass" aria-expanded="false" data-toggle="dropdown" class="btn btn-xs  btn-warning " type="button">History&nbsp;
  ...
  <tr>
   <td colspan="2" height="38">
     ...
     <form method="post" enctype="multipart/form-data"  action="upload.php">
       <span class="btn btn-default btn-xs btn-info  btn-file" style="float: left;padding:5px">
         Attachment  <span class="glyphicon glyphicon-paperclip"></span>
...
<div class="msg_nb_logo"></div>
 <ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header1 ui-corner-all">
		<li id="welcome" style="font-size: 54px; height: 542px; text-align: center;line-height:220px;">Welcome</li> 
</ul>
...
function ChangeStatus123(val1,val2){
	//alert(val);
    
    $.post('<?php echo URL ?>change_status.ajax.php', {add_chatuser: 'true',id:val1,appid:val2,regularapp:true}, function(data, textStatus, xhr) {
		 window.location.href = window.location.href;
    });
 ...
 /* function ChangeStatus123Regular(val){
	//alert(val);
    
    $.post('<?php echo URL ?>change_status.ajax.php', {add_chatuser: 'true',appid:val,regularapp:true}, function(data, textStatus, xhr) {
		 window.location.href = window.location.href;
    });
