...
if(isset($_SESSION['loginType']) && $_SESSION['loginType']=='D'){ 
		...
		//$cur_date=date('Y-m-d');
		$cur_time=date("H:i:s");
		$doctorAvailabityDetssql="select * from doctor_availability where doctor_id = '".$doctorid."' and status =1 and date(app_date)='".$created."' and  start_time <='".$cur_time."' and end_time >= '".$cur_time."' limit 1";
		//echo $doctorAvailabityDetssql;exit;
		$doctorAvailabityDets=mysql_query($doctorAvailabityDetssql)or die(mysql_error());
		if(mysql_num_rows($doctorAvailabityDets)>0){
			$docAvailabiltyDetails=mysql_fetch_assoc($doctorAvailabityDets);
			$appointment_availbility=$docAvailabiltyDetails['id'];
			
			$appntdets=mysql_query("select * from appointments where doctorid='".$doctorid."' and appointment_date = '".$created."' and status=1 and join_status=2 and appointment_availbility='".$_SESSION['AvailabityTimeId']."' and appoint_book_slut='".$_SESSION['AvailabitySlotId']."'")or die(mysql_error());
			if(mysql_num_rows($appntdets)>0){
				$appointDets=mysql_fetch_assoc($appntdets);
				$patienid=$appointDets['patientid'];
				 $appoint_id = $appointDets['id'];
			}
		}
...
if(isset($_SESSION['loginType']) && $_SESSION['loginType']=='P'){
	...
	$cur_date=date('Y-m-d');
	$cur_time=date("H:i:s");
	$appointDetsql= "select * from appointments where patientid='".$patientid."' and appointment_date = '".$cur_date."' and status=1 and join_status=2 and appointment_availbility='".$_SESSION['AvailabityTimeId']."' and appoint_book_slut='".$_SESSION['AvailabitySlotId']."'";
	//echo $appointDetsql;exit;
	$appointDetails=mysql_query($appointDetsql)or die(mysql_error());
	if(mysql_num_rows($appointDetails)>0){
		
		while($appointDetails = mysql_fetch_assoc($appointDetails)){
			$availabityId=$appointDetails['appointment_availbility'];
		 	 $doc_id=$appointDetails['doctorid'];
			  $appoint_id = $appointDetails['id'];
			$checkAvailability=mysql_query("select * from doctor_availability where id='".$availabityId."' and doctor_id = '".$doc_id."' and status =1 and date(app_date)='".$cur_date."' and  start_time <='".$cur_time."' and end_time >= '".$cur_time."' limit 1");
			 if(mysql_num_rows($checkAvailability)>0){
				 $availabityDet=mysql_fetch_assoc($checkAvailability);
				 $doctorId=$availabityDet['doctor_id'];
...
filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#2fb2c4', endColorstr='#02adc6',GradientType=0 ); /* IE6-9 */
  ...
  #margindel-video{
	float:left; width:100%; text-align:center; margin-top:150px;
			...
			/*#usercam .media-controls, .volume-control{ display:none !important;}*/
			/*.media-controls:nth-child(2){ display:none;}*/
    ...
    <div class="container">
        <div class="row ">
            ...
            <div class="col-md-8 col-sm-8 header_nav">
                    ...
                    <div class="rmm style">
                        <ul>
                            <li><a href="<?php echo SITEURL.'/inhealth/dashboard';?>">Dashboard</a></li>
                            <li><a href="<?php echo SITEURL.'/inhealth/pages/about-us';?>">About Us</a></li>
...
<!--<div class="pl j-pl hidden">-->
            ...
            <script>
                                ...
                                <div>
                                    <div class="logininnerpage_right">
                                                ...
                                                <?php if(isset($_SESSION['loginType']) && $_SESSION['loginType']=='D'){ 
														...
														$created=date('Y-m-d');
														$patientTestDetailsql="select * from vitalsign_result where doctorid='".$doctorid."' and patientid='".$patientid."' and appoint_type =0 and appoint_id='".$appoint_id."' and date(created)='".$created."'";
              ...
              <iframe src="<?php echo SITEURL.'/chat/';?>" height="590px" width="100%" border=0 scrolling="no"></iframe>
...
<script type="text/javascript">
	...
	$('#test_type').change(function() {
        ...
        $.ajax({
                   type:"POST",
                   url:"<?php echo BASEURL;?>ajax/ajax.php",
                   data:dataVal,
...
$(document).ready(function(){
			...
			setInterval(setendcallbtn,2000);
			setInterval(check_connection,5000);
	});
function check_connection(){
	var dataVal = "task=check_appoint_conection";
	$.ajax({
			   type:"POST",
			   url:"<?php echo BASEURL;?>ajax/ajax.php",
			   data:dataVal,
...
$('#success1').html("<h4 style='color:green;'>Waiting To Save Test Results ....</h4>");   
$.ajax({
url: '<?php echo BASEURL;?>ajax/ajax.php?task=testformSubmit', // Url to which the request is send
