...
if(isset($_SESSION['loginType']) && $_SESSION['loginType']=='D'){ 
		...
		if(mysql_num_rows($doctorAvailabityDets)>0){
			...
			$appntdets=mysql_query("select * from appointments where doctorid='".$doctorid."' and appointment_date = '".$created."' and status=1 and appointment_availbility='".$appointment_availbility."'")or die(mysql_error());
...
if(isset($_SESSION['loginType']) && $_SESSION['loginType']=='P'){
	...
	$cur_date=date('Y-m-d');
	$cur_time=date("H:i:s");
	$appointDetsql= "select * from appointments where patientid='".$patientid."' and appointment_date = '".$cur_date."' and status=1";
...
<!--<div class="pl j-pl hidden">-->
            ...
            <script>
              ...
              <iframe src="<?php echo 'http://'.$_SERVER['HTTP_HOST'].'/inhealth/chat/';?>" height="727px" width="100%" border=0 scrolling="no"></iframe>
