...
if(isset($_REQUEST['task']) && $_REQUEST['task'] == 'testformSubmit')
{
	$doctorid=$_SESSION['doctorID'];
		...
		$appoint_type=0;	//appointment Based Appointment
...
if(isset($_REQUEST['task']) && $_REQUEST['task'] == 'getTestresult')
{
	...
	if(mysql_num_rows($patientTestDet)>0){
		while($patientTestDetails = mysql_fetch_assoc($patientTestDet)){ //print_r($patientTestDet);
			...
			if($patientTestDetails['ecg']!=""){
				$reff=SITEURL."/files/vital_sign_reports/".stripslashes($patientTestDetails['ecg']);
			}else{$reff="javascript:void(0);";}
				  ...
				  $result= "<tr><th width='15%' height='27' align='left'>Temperature</th>
                    ...
                    <tr height='5px'><td colspan='3'></td></tr>
                     ...
                     <tr><th width='15%' height='27' align='left'>ECG</th>
                        <td width='1%'>:</td>
    					<td><a href='".$reff."' target='_blank'>Click Here To View Report</a></td>
                    </tr>
...
{
   $updatequery=mysql_query('update appointments set join_status=3 where id="'.$_SESSION['appoint_id'].'"')or die(mysql_error());
   if($updatequery){
	   echo 0;	
   }else{
	   echo 1;
...
{
   $appointDet=mysql_query('select * from appointments where id="'. $_SESSION['appoint_id'].'"')or die(mysql_error());
	if(mysql_num_rows($appointDet)>0){
		$appntDets=mysql_fetch_assoc($appointDet);
		 $joinStats=$appntDets['join_status'];
...
if(isset($_REQUEST['task']) && $_REQUEST['task'] == 'check_appoint_conection')
{
   $checkAppointJoinStat=mysql_query('select * from appointments where id="'. $_SESSION['appoint_id'].'"')or die(mysql_error());
   $appntJoinDets=mysql_fetch_assoc($checkAppointJoinStat);
   $joinStat=$appntJoinDets['join_status'];
   if($joinStat==3){
	   echo 0;	
   }else{
	   echo 1;
